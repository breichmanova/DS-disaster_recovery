...
def clean_data(df):
    ...
    tmp_cat = df[['id','categories']]
    categories = tmp_cat['categories'].str.split(';', expand = True)
    categories = pd.concat([tmp_cat,categories], axis=1)
    categories = categories.drop('categories', axis = 1)
    
    # select the first row of the categories dataframe
    row = list(categories.iloc[0,1:])

    # extract a list of new column names for categories
    category_colnames = ['id']
    for string in row:
        tmp = re.sub('-.+','', string)
        category_colnames.append(tmp)
    
    # rename the columns of `categories`
    categories.columns = category_colnames
    
    for column in category_colnames[1:]:
